    ...
    const deleteProduct = url => {
        axios.delete("http://localhost:8000/api" + url)
            .then(response => {
                ...
                console.log("Product was successfully deleted: " + response + " count: " + counter.count)
            })
                ...
                <div className="col-6 offset-3 text-center">
                    {
                        products.map((item, i) =>
                            <p key={i} style={{marginTop: "10px"}}>
                                ...
                                <button onClick={(e) => {
                                    deleteProduct(getProductUrl(item))
                                }} className="btn btn-danger btn-sm" style={{marginLeft: "20px"}}>delete
                                </button>
                            </p>
                        )
                    }
